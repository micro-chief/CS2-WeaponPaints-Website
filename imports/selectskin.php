...
// Умная локальная заглушка с поддержкой кастомных аватарок
$steamid = isset($_SESSION['steamid']) ? $_SESSION['steamid'] : 'Guest';
$custom_avatar_path = "src/avatars/" . $steamid . ".png";

$UserInfo = new stdClass();
$UserInfo->personaname = "Игрок #" . substr($steamid, -4);

// Если файл с аватаркой игрока существует на сервере — показываем его, иначе — дефолтный нож
if (file_exists(dirname(__DIR__) . '/' . $custom_avatar_path)) {
    $UserInfo->avatarfull = "/" . $custom_avatar_path;
} else {
    $UserInfo->avatarfull = "/src/img/skins/weapon_knife.png";
}
...
<footer>
    <div class="wrapper">
        <!-- Блок профиля игрока -->
        <div class="info">
            <!-- Форма загрузки кастомной аватарки -->
            <form id="avatarForm" action="/imports/upload_avatar.php" method="POST" enctype="multipart/form-data" style="display: inline-block; margin-right: 10px; vertical-align: middle; margin-bottom: 0;">
                <label for="avatarInput" style="cursor: pointer; display: block;" title="Нажмите, чтобы изменить аватарку">
                    <img src="<?= $UserInfo->avatarfull ?>" alt="avatar" style="width: 40px; height: 40px; border-radius: 50%; border: 1px solid #444; display: block;">
                </label>
                <input type="file" id="avatarInput" name="avatar" accept="image/png, image/jpeg" style="display: none;" onchange="document.getElementById('avatarForm').submit();">
            </form>
            
            <!-- Текст приветствия -->
            <p style="display: inline-block; vertical-align: middle; margin: 0;">
                <?= str_replace('{{name}}', '<strong>'.$UserInfo->personaname.'</strong>', $translations->skins->footer->signedin); ?>
            </p>
        </div> <!-- Закрываем блок info -->

        <!-- Блок копирайта автора (строки 492-494) -->
        <div class="credit">
            <p>This website created by LielXD, and modified by igsveff</p>
        </div>
        <div class="actions">
            <div class="settings">
