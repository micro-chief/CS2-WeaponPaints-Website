...
// Финальная железобетонная заглушка для работы с аватарками
$steamid = isset($_SESSION['steamid']) ? $_SESSION['steamid'] : 'Guest';
$custom_avatar_file = "/src/avatars/" . $steamid . ".png";

$UserInfo = new stdClass();
$UserInfo->personaname = "Игрок #" . substr($steamid, -4);

// Проверяем наличие файла на диске от самого корня сервера XAMPP
if (file_exists($_SERVER['DOCUMENT_ROOT'] . $custom_avatar_file)) {
    // Если картинка загружена — отдаем путь от корня домена
    $UserInfo->avatarfull = $custom_avatar_file;
} else {
    // Если картинки нет — отдаем дефолтный нож от корня домена
